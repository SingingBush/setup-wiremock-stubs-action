...
Posts Wiremock stubs to a Wiremock server running in a container. This action is intended to be used
in conjunction with wiremock running as a GH service
...
1. Then package the project for distribution using `npm run bundle` (changes to dist must be
   committed)
1. Run the tests using `npm test`
1. Check code linter using `npm run format:check` or use `format:write` to apply the changes.
   (linting must pass to have changes merged)
